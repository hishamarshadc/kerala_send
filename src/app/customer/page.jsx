...
import { submitParcelRequest, fetchNotifications } from "@/app/actions";
...
export default function CustomerPage() {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
        ...
        // Filter mock: only show ones for "my" parcels (simulated by checking if notification exists)
        // In real app, filter by user ID. Here we just show all for demo.
        if (notifs.length > notifications.length) {
             // Play sound or toast? Just update state for now.
        }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!locations.pickup || !locations.drop) return;

    await submitParcelRequest({
      customerName: "Demo User",
      pickup: { ...locations.pickup, name: "Selected Location" },
      drop: { ...locations.drop, name: "Selected Location" },
                    ...
                    <GlassCard className="animate-in fade-in slide-in-from-left-4 duration-500">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            {calcResult && (
                                <div className="mt-6 p-4 bg-white/40 rounded-xl border border-white/60">
                                    ...
                                    <p className="text-xs text-gray-400 mt-2 text-center">
                                        (Rate: ₹4/km light, ₹7/km heavy. Long distance &gt;200km flat ₹4/km)
                                    </p>
                                </div>
                            )}
                            ...
                            <GlassButton 
                                ...
                                type="submit"
                                disabled={!locations.pickup || !locations.drop}
                            >
                                List Parcel
                            </GlassButton>
                        </form>
